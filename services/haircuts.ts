import {
  ...
  export async function uploadHaircut(file: File, jwt: string): Promise<Record<string, string>> {
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(`Failed to upload haircut: ${errorJson.message || response.statusText}`);
        } catch (e) {
          // If not JSON or parsing fails, use the raw text
          throw new Error(`Failed to upload haircut: ${response.status} ${response.statusText} - ${errorText}`);
        }
      }
...
export async function getHaircutEvents(params?: {
  ...
  export async function deleteHaircutEvent(id: string, jwt: string): Promise<void> {
    try {
      const response = await fetch(`${getValue('API')}haircuts/events/${id}`, {
        method: 'DELETE',
        ...addAuthHeader(jwt)
