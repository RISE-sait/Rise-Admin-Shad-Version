...
import { CreateFeatureCardRequest, FeatureCard } from "@/types/website-promo";
import { createFeatureCard, updateFeatureCard, uploadPromoImage } from "@/services/website-promo";
import { LayoutGrid, Upload, Loader2 } from "lucide-react";
...
export default function FeatureCardForm({ featureCard, onSuccess }: FeatureCardFormProps) {
  const { user } = useUser();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [previewUrl, setPreviewUrl] = useState<string>(featureCard?.image_url || "");

  const isEditing = !!featureCard;
  ...
  } = useForm<FeatureCardFormValues>({
    defaultValues: {
      title: featureCard?.title || "",
      description: featureCard?.description || "",
      image_url: featureCard?.image_url || "",
      button_text: featureCard?.button_text || "",
      button_link: featureCard?.button_link || "",
      display_order: featureCard?.display_order || 1,
      is_active: featureCard?.is_active ?? true,
      start_date: featureCard?.start_date?.split("T")[0] || "",
      end_date: featureCard?.end_date?.split("T")[0] || "",
    },
  });
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      const url = await uploadPromoImage(file, "feature", user.Jwt);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const payload: CreateFeatureCardRequest = {
      title: values.title,
    ...
    try {
      if (isEditing && featureCard) {
        await updateFeatureCard(featureCard.id, payload, user.Jwt);
        toast.success("Feature card updated successfully");
      } else {
        await createFeatureCard(payload, user.Jwt);
        toast.success("Feature card created successfully");
      }
      await onSuccess?.();
    } catch (error) {
      console.error("Failed to save feature card:", error);
      toast.error(`Failed to ${isEditing ? "update" : "create"} feature card. Please try again.`);
    }
  });
