...
export default function FamilyTab({
  ...
  useEffect(() => {
    ...
    // Always try to load children (a parent might also be a child)
    fetchChildren();
  ...
  const handleUnlinkChild = async () => {
    ...
    setIsUnlinking(true);
    try {
      const error = await unlinkChildFromParent(childToUnlink.id, user.Jwt);
      if (error) {
        toast({
          status: "error",
          description: error,
  ...
  const handleUnlinkFromParent = async () => {
    ...
    try {
      const error = await unlinkChildFromParent(customer.id, user.Jwt);
      if (error) {
        toast({
          status: "error",
          description: error,
  ...
  return (
    <div className="space-y-6">
      {/* Parent Info Section (shown when this customer is a child) */}
      {isChild && (
          ...
          {isLoadingChildren ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Use the buttons above to link an existing customer or create a
                new child account
              </p>
            </div>
          ) : (
                      ...
                      {child.memberships && child.memberships.length > 0 && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {child.memberships[0].membership_plan_name ||
                              child.memberships[0].membership_name}
                          </span>
                          {child.memberships[0].subscription_status && (
                            <Badge
                              variant={
                                child.memberships[0].subscription_status ===
                                "active"
